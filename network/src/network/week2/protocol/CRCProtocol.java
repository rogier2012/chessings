...
public class CRCProtocol implements IRDTProtocol {
		...
		if (this.role == Role.Sender) {
			...
			for (int counter = 0; counter < 5; counter++) {
				while (!stop) {
					...
					// read (packetToSend.length) bytes and store them in the
					// packet
					...
					System.out.println("Sending packet: " + k);
					...
					if (filePointer >= fileContents.length) {
						System.out
								.println("Reached end-of-file. Done sending.");
						stop = true;
					}
